...
class Pcomp(Block):
    ...
    def produce_rising(self, current_pulse):
        ...
        if self.RELATIVE:
            ...
            if self.PRE_START > 0:
                if abs(self.INP - self.initial) >= self.PRE_START:
                    ...
                    sign = np.sign(self.initial - self.INP)
            elif self.START > 0:
                if abs(self.INP - self.initial) >= self.START:
                    sign = np.sign(self.INP - self.initial)
            else:
    ...
    def do_rising_check(self):
        ...
        if self.reached_thresh(self.INP, thresh):
            return self.produce_rising(thresh)
        else:
            return WAIT_RISING
    ...
    def on_changes(self, ts, changes):
        ...
        if self.ACTIVE:
            if self.STATE == WAIT_DIR:
                ...
                self.STATE = self.do_check_dir()

            if self.STATE == WAIT_PRE_START:
                # Check when we have passed START - PRE_START
                self.STATE = self.do_pre_start_check()

            if self.STATE == WAIT_RISING:
                # Check when we have passed START
                self.STATE = self.do_rising_check()
            elif self.STATE == WAIT_FALLING:
                # Check when we have passed WIDTH
                self.STATE = self.do_falling_check()
