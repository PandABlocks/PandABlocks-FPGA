...
A ``wave.fst`` file can be found in each tests subdirectory. This can be opened
...
Testtarget is a target created to facilitate testing the full hierarchy of
entities, including auto-generated code and all infrastructure supporting DMA
transfers.

Two resources rely on testtarget: system tests and simulation server.

System tests
~~~~~~~~~~~~

These are tests to cover interactions between all entities in several use-cases,
the main ones being: register accesses, PCAP captures and a PGEN run.

Before running the system tests, you need to set ``APP_NAME = testtarget`` in
``CONFIG``, then run::

    make cocotb_system_tests

Simulation server
~~~~~~~~~~~~~~~~~

It listens to TCP port 9999 waiting for the panda server to connect, when this
happens, it receives commands and do the appropriate interaction with the
simulated VHDL code. In order to start the FPGA simulation server, first make
sure you have ``APP_NAME = testtarget`` in ``CONFIG``, then run::

    make run_sim_server

Then, run the panda server passing the relevant target information::

   ./server_build/sim_server/sim_server -c /fpga_build/apps/testtarget/autogen/config_d/ -R
