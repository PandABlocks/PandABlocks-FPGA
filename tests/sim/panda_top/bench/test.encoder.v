...
REG_WRITE(OUTENC_BASE, OUTENC_PROTOCOL, 1);         // SSI
repeat(15000) @(posedge tb.uut.ps.FCLK);
REG_WRITE(OUTENC_BASE, OUTENC_BITS, 24);
REG_WRITE(OUTENC_BASE, OUTENC_VAL, 1);

REG_WRITE(INENC_BASE, INENC_PROTOCOL, 1);           // SSI
repeat(15000) @(posedge tb.uut.ps.FCLK);
REG_WRITE(INENC_BASE, INENC_BYPASS, 1);
REG_WRITE(INENC_BASE, INENC_CLK, BITS_ONE0);
